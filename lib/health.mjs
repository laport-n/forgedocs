...
import { parseCodemap } from './diff.mjs'
...
const CHECKS = [
  ...
  { id: 'agent-instructions', label: 'Agent instruction file present', points: 5, category: 'Structure' },
  ...
  for (const check of CHECKS) {
    ...
    switch (check.id) {
      ...
      case 'agent-instructions': {
        const { found, files } = findInstructionFiles(repoPath)
        if (found) {
          result.earned = check.points
          result.status = 'pass'
          result.detail = files.join(', ')
        } else {
          result.detail = 'Missing — AI agents lack context. Run forgedocs install'
        }
        break
      }
      case 'docs-dir': {
      ...
      case 'codemap': {
        ...
        if (fs.existsSync(archPath)) {
          ...
          if (codemap.length > 0) {
            result.earned = check.points
            result.status = 'pass'
            result.detail = `${codemap.length} entry(ies) in codemap`
          } else {
            result.detail = 'No Codemap entries found'
          }
        } else {
          result.detail = 'No ARCHITECTURE.md'
        }
      ...
      case 'service-map-fresh': {
        ...
        if (fs.existsSync(smPath)) {
          ...
          if (match) {
            ...
            result.status = 'warn'
            result.detail = 'No "Last verified" date — add one to track freshness'
          }
        } else {
