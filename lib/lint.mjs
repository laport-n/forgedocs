...
export function lintDocs(repoPath) {
  ...
  if (fs.existsSync(archPath)) {
    ...
    // Rule 2: Stale placeholders in ARCHITECTURE.md
    const placeholders = content.match(/\[To be documented[^\]]*\]/g)
    if (placeholders) {
      add(
        'stale-placeholder',
        'error',
  ...
  if (fs.existsSync(docsDir)) {
    ...
    for (const mdFile of mdFiles) {
      ...
      const pathRefs = content.matchAll(/`((?:src|lib|app|bin|pkg|internal|cmd)\/[^`\s]+)`/g)
      for (const match of pathRefs) {
        const refPath = match[1]
        if (!fs.existsSync(path.join(repoPath, refPath))) {
          add(
            'broken-ref',
            'error',
            relFile,
