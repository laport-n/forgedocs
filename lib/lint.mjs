...
export function lintDocs(repoPath) {
  ...
  if (fs.existsSync(readmePath) && fs.existsSync(archPath)) {
    ...
    const cliEntry = parseCodemap(archContent).find(
      (e) => e.component === 'CLI' || e.purpose?.includes('subcommands'),
    )
    if (cliEntry) {
  ...
  if (fs.existsSync(readmePath) && fs.existsSync(claudeCommandsDir)) {
    ...
    const countMatch = readmeContent.match(/(\d+)\s+(?:AI\s+)?commands?\s+for\s+Claude/i)
