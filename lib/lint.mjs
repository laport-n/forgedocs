...
import { findInstructionFiles } from './agents.mjs'
...
export function lintDocs(repoPath) {
  ...
  if (fs.existsSync(archPath)) {
    ...
    for (const entry of codemap) {
      ...
      const checkPath = entry.path.includes('*')
        ? entry.path.split('*')[0].replace(/\/$/, '')
        : entry.path.replace(/\/$/, '')
  ...
  // Rule 5: Agent instruction file checks (CLAUDE.md, .cursorrules, etc.)
  const { found: hasAnyInstructionFile, files: instructionFiles } = findInstructionFiles(repoPath)

  // Check CLAUDE.md specifically for navigation structure (it has the strictest format)
  ...
  // Check all other instruction files for stale placeholders
  for (const file of instructionFiles) {
    ...
    if (placeholders) {
      add(
        ...
        file,
        `${placeholders.length} placeholder(s) found`,
        'Fill in sections or run `forgedocs sync-agents`',
      )
    }
  }

  if (!hasAnyInstructionFile) {
    add(
      'no-agent-instructions',
      'warn',
      '(none)',
      'No agent instruction file found (CLAUDE.md, .cursorrules, etc.)',
      'Run `forgedocs install --agents auto` or `forgedocs quickstart`',
    )
  }
  ...
  // Check docs/ directory AND root-level doc files (ARCHITECTURE.md, README.md, and all instruction files)
  const rootDocCandidates = ['ARCHITECTURE.md', 'README.md', ...instructionFiles]
  const rootDocFiles = [...new Set(rootDocCandidates)]
  ...
  // Rule 11: CHANGELOG freshness — if CHANGELOG.md exists, verify latest version has an entry
  const changelogPath = path.join(repoPath, 'CHANGELOG.md')
  const pkgPath = path.join(repoPath, 'package.json')
  if (fs.existsSync(changelogPath) && fs.existsSync(pkgPath)) {
    try {
      const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'))
      const changelog = fs.readFileSync(changelogPath, 'utf-8')
      if (pkg.version && !changelog.includes(pkg.version)) {
        add(
          'changelog-stale',
          'warn',
          'CHANGELOG.md',
          `No entry found for current version ${pkg.version}`,
          `Add a changelog entry for v${pkg.version}`,
        )
      }
    } catch {
  ...
  // Rule 12: Glossary term freshness — check for placeholder definitions
  ...
  if (fs.existsSync(glossaryPath)) {
    ...
    if (placeholders) {
      add(
        ...
        'docs/glossary.md',
        `${placeholders.length} placeholder(s) found`,
        'Fill in glossary definitions',
      )
    }
  }

  // Rule 13: Security doc — check for common anti-patterns if security.md defines rules
  const securityPath = path.join(repoPath, 'docs', 'security.md')
  if (fs.existsSync(securityPath)) {
    const secContent = fs.readFileSync(securityPath, 'utf-8')
    // If security.md says "no eval" or similar, grep for violations
    if (/no\s+eval|never.*eval|avoid.*eval/i.test(secContent)) {
      ...
      for (const srcFile of sourceFiles) {
        ...
        if (/\beval\s*\(/.test(src) || /\bnew\s+Function\s*\(/.test(src)) {
          const relSrc = path.relative(repoPath, srcFile)
          add(
            'security-violation',
            'error',
            relSrc,
            'Uses eval() or new Function() — violates docs/security.md rules',
            'Remove eval/Function usage or update security.md',
          )
        }
      }
    }
    // If security.md says "no credentials" or "no secrets", check for common patterns
    if (/no\s+credential|no\s+secret|never.*store.*(key|token|secret)/i.test(secContent)) {
      const sourceFiles = collectSourceFiles(repoPath)
      for (const srcFile of sourceFiles) {
        const src = fs.readFileSync(srcFile, 'utf-8')
        if (/(?:API_KEY|SECRET_KEY|PRIVATE_KEY|PASSWORD)\s*=\s*['"][^'"]+['"]/i.test(src)) {
          const relSrc = path.relative(repoPath, srcFile)
          add(
            'security-violation',
            'warn',
            relSrc,
            'Possible hardcoded credential — violates docs/security.md rules',
  ...
  // Rule 14: README ↔ ARCHITECTURE.md CLI command consistency
  // If ARCHITECTURE.md has a codemap and README has a CLI Reference table, check they list the same commands
  if (fs.existsSync(readmePath) && fs.existsSync(archPath)) {
    const readmeContent = fs.readFileSync(readmePath, 'utf-8')
    const archContent = fs.readFileSync(archPath, 'utf-8')

    // Extract CLI subcommands from ARCHITECTURE.md codemap CLI entry
    const cliEntry = parseCodemap(archContent).find((e) => e.component === 'CLI' || e.purpose?.includes('subcommands'))
    if (cliEntry) {
      const archSubcmds = cliEntry.purpose.match(/\(([^)]+)\)/)?.[1]
      if (archSubcmds) {
        const archCmdSet = new Set(archSubcmds.split(',').map((c) => c.trim()))

        // Extract commands from README CLI Reference table (lines like | `forgedocs <cmd>` |)
        const readmeCmds = new Set()
        for (const match of readmeContent.matchAll(/\|\s*`forgedocs\s+(\w+)/g)) {
          readmeCmds.add(match[1])
        }

        if (readmeCmds.size > 0) {
          for (const cmd of archCmdSet) {
            if (!readmeCmds.has(cmd) && !['help'].includes(cmd)) {
              add(
                'readme-missing-cmd',
                'warn',
                'README.md',
                `CLI command "${cmd}" is in ARCHITECTURE.md codemap but missing from README CLI Reference`,
                'Add the command to the README CLI Reference table',
              )
            }
          }
          for (const cmd of readmeCmds) {
            if (!archCmdSet.has(cmd)) {
              add(
                'readme-extra-cmd',
                'warn',
                'README.md',
                `CLI command "${cmd}" is in README but not in ARCHITECTURE.md codemap`,
                'Add the command to the ARCHITECTURE.md CLI entry or remove from README',
              )
            }
          }
        }
  ...
  // Rule 15: README ↔ templates — Claude commands count consistency
  const claudeCommandsDir = path.join(repoPath, 'templates', 'claude-commands')
  if (fs.existsSync(readmePath) && fs.existsSync(claudeCommandsDir)) {
    const readmeContent = fs.existsSync(readmePath) ? fs.readFileSync(readmePath, 'utf-8') : ''
    const templateCount = fs.readdirSync(claudeCommandsDir).filter((f) => f.endsWith('.md')).length

    // Check if README mentions a command count that doesn't match
    const countMatch =
      readmeContent.match(/(\d+)\s+(?:AI\s+)?commands?\s+for\s+Claude/i) ||
      readmeContent.match(/(\d+)\s+Claude\s+(?:Code\s+)?commands/i)
    if (countMatch) {
      const readmeCount = Number.parseInt(countMatch[1], 10)
      if (readmeCount !== templateCount) {
        add(
          'readme-cmd-count',
          'warn',
          'README.md',
          `Claims ${readmeCount} Claude commands but templates/ has ${templateCount}`,
          `Update the count in README.md to ${templateCount}`,
        )
      }
    }
  }
...
function collectSourceFiles(repoPath) {
  const EXTENSIONS = new Set(['.js', '.mjs', '.cjs', '.ts', '.mts', '.jsx', '.tsx', '.py', '.rb'])
  const SKIP = new Set([
    'node_modules',
    'vendor',
    'dist',
