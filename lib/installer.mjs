...
export function installTemplates(templatesDir, targetRepo, { force = false } = {}) {
  ...
  if (!settings.hooks?.PreToolUse?.some((h) => h.hook?.includes('forgedocs hook'))) {
    settings.hooks = settings.hooks || {}
    settings.hooks.PreToolUse = settings.hooks.PreToolUse || []
    settings.hooks.PreToolUse.push({
      matcher: 'Bash(*git push*)',
      hook: 'npx forgedocs hook pre-push',
      description: 'Check documentation health before pushing',
    })
    settingsChanged = true
    installed.push('.claude/settings.json (pre-push doc check hook)')
  } else {
    skipped.push('.claude/settings.json (pre-push hook already configured)')
  }
