...
| Rule | Check |
|------|-------|
| All tests pass | \`npm test\` or equivalent → exit 0 |
| No secrets in source | \`grep -rE "(password|secret|token)\\s*=" --include="*.{js,ts,py,rb,go}" . \\| grep -v node_modules \\| grep -v test \\| wc -l\` → should be 0 |
| [Add project-specific invariants here] | \`[verification command]\` |
...
See [ARCHITECTURE.md](ARCHITECTURE.md) for system design, codemap, and verifiable invariants.
...
## Where things live
- [To be filled — e.g. \`src/\` — application source code]
- [To be filled — e.g. \`test/\` — test suite]
- [To be filled — e.g. \`config/\` — configuration files]

## What to never do
- [To be filled — e.g. Never commit secrets or credentials]
- [To be filled — e.g. Never bypass auth middleware]

## How to run
- [To be filled — e.g. \`npm test\` — run all tests]
- [To be filled — e.g. \`npm run dev\` — start dev server]

## When to update documentation
- Adding/removing a module → update \`ARCHITECTURE.md\` codemap
 ...
 */
export function quickstart(targetDir, templatesDir, { preset = null, force = false, agents = 'auto' } = {}) {
  const created = []
  const skipped = []
  ...
  if (templatesDir && fs.existsSync(templatesDir)) {
    installResult = installTemplates(templatesDir, targetDir, { force, agents })
  }
