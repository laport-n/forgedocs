...
export function installTemplates(templatesDir, targetRepo, { force = false } = {}) {
  ...
  // Configure post-commit audit hook
  const hookScriptPath = path.join('.claude', 'hooks', 'post-commit-audit.sh')
  const hasPostCommitHook = settings.hooks?.PostToolUse?.some((h) =>
    h.hooks?.some((hook) => hook.command?.includes('post-commit-audit')),
  )
  if (!hasPostCommitHook) {
    ...
    settingsChanged = true
    installed.push('.claude/settings.json (post-commit audit hook)')
  } else {
    skipped.push('.claude/settings.json (post-commit hook already configured)')
  }
