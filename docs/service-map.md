...
|----------|-----------------------|----------|
| MCP clients (Claude Code, Cursor, Windsurf, Cline) | Query docs via `forgedocs mcp` | JSON-RPC 2.0 over stdio |
| Claude Code (Commands) | Runs `/doc-init`, `/doc-sync`, etc. via installed templates | Markdown prompt files in `.claude/commands/` |
| Cursor / Windsurf / Copilot / Cline | Read generated instruction files at session start | `.cursor/rules/forgedocs.mdc`, `.windsurfrules`, `.github/copilot-instructions.md`, `.clinerules` |
...
Developer
  ...
  │
  ├── AI agents (Claude Code, Cursor, Windsurf, Copilot, Cline)
  │     ├── MCP Server (forgedocs mcp) → JSON-RPC 2.0
  │     ├── Instruction files (CLAUDE.md, .cursorrules, etc.) → generated by lib/instruction-gen.mjs
  │     └── Claude-only: .claude/commands/ prompt templates + skills + hooks
  │
  └── GitHub Actions
